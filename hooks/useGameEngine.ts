...
export type CreationPhase = 'initializing' | 'colors' | 'world' | 'characters' | 'avatar' | 'finalizing' | null;

// Processing phase type for message progress tracking
export type ProcessingPhase = 'classifying' | 'generating' | 'updating' | null;
...
export const useGameEngine = (): UseGameEngineReturn => {
				...
				if (fullStory && loadingStoryRef.current === currentStoryId) {
					...
					setStories((prev) =>
						prev.map((s) => {
							...
							const mergedMessages = sanitizeMessages([...s.messages, ...dbMessagesNotInState]);
	...
	const updateNarrativeStyle = async (mode: NarrativeStyleMode, customStyle?: string) => {
		...
		safeUpdateStory((story) => {
			...
			return {
				...story,
				config: nextConfig,
				narrativeConfig: nextNarrativeConfig,
			};
		});
	};
			...
			const newMessages: ChatMessage[] = (initResponse.messages || []).map((m: any, idx: number) => {
				...
				if (m.type === 'system') {
					...
					if (charName && charName !== 'Narrator') {
						...
						generatedCharacters.forEach((c: any) => {
							charsRecord[c.id] = c;
						});
		...
		try {
			...
			safeUpdateStory((prev) => {
						...
						if (u.id && next.characters[u.id]) {
							...
							next.characters[u.id] = {
								...
								relationships: u.relationships
									? { ...existing.relationships, ...u.relationships }
				...
				const isForbiddenSpeaker = (speaker?: string | null) => {
					...
					if (
						significantTokens.some((token) => normalizedSpeaker.includes(token) || token.includes(normalizedSpeaker))
					) {
						return true;
					}
					return false;
				...
				const botMessages: ChatMessage[] = filteredMessages.map((m: GMResponseMessage, idx: number) => {
					...
					if (m.type === 'system') {
						...
						if (char) {
							...
							console.warn(
								`[Character Lookup] Character "${charName}" not found in characters map, using name as senderId`,
							);
	...
	const handleExportJourney = async () => {
		...
		const fileName = `infinitum_${
			activeStoryData?.title.replace(/[^a-zA-Z0-9]/g, '_') || 'journey'
		}_${Date.now()}.json`;
